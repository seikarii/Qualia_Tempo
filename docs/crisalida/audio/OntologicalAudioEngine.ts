...
import type { QualiaState, PredictiveEntityState } from '../core/types';
...
export interface EmergentBehavior {
  ...
  entities: PredictiveEntityState[];
  strength?: number;
  description?: string;
  timestamp?: number;
}
...
export class OntologicalAudioEngine {
  ...
  public createEntityVoice(entityId: string, qualiaState: QualiaState): void {
    ...
    const synth = new Tone.PolySynth(Tone.Synth, {
      oscillator: {
        type: this.getOscillatorType(qualiaState.consciousness_density),
        modulationIndex: qualiaState.narrative_importance * 10,
      },
      envelope: {
        attack: 0.07 + qualiaState.temporal_coherence * 0.35,
        decay: 0.16,
        sustain: 0.32 + qualiaState.consciousness_density * 0.5,
        release: 0.5 + qualiaState.temporal_coherence * 1.3
      },
      volume: -7 + (qualiaState.narrative_importance ?? 0) * 7
    });
  ...
  public updateEntitySound(entityId: string, qualiaState: QualiaState): void {
    ...
    const baseFreq = this.mapConsciousnessToFrequency(qualiaState.consciousness_density);
    const harmonic = this.mapValenceToHarmonic(qualiaState.emotional_valence);
    const rhythm = this.mapArousalToRhythm(qualiaState.arousal);

    if (qualiaState.arousal > 0.2) {
      const note = Tone.Frequency(baseFreq * harmonic).toNote();
      const velocity = Math.min(qualiaState.arousal + (qualiaState.narrative_importance ?? 0) * 0.25, 1.0);
  ...
  /**
   * Determina el tipo de oscilador según la densidad de consciencia.
   */
  private getOscillatorType(consciousnessDensity: number): OscillatorType {
    if (consciousnessDensity > 0.9) return "sine";
    if (consciousnessDensity > 0.7) return "triangle";
    if (consciousnessDensity > 0.4) return "sawtooth";
    if (consciousnessDensity > 0.2) return "pulse";
    return "square";
  }

  /**
   * Mapea la consciencia a una frecuencia audible.
   */
  private mapConsciousnessToFrequency(consciousness: number): number {
    // Rango extendido para mayor expresividad
    return 110 + consciousness * 2100;
  }
  ...
  /**
   * Mapea el nivel de activación a una duración rítmica.
   */
  private mapArousalToRhythm(arousal: number): string {
    if (arousal > 0.8) return "16n"; // Muy activado: notas rápidas
    if (arousal > 0.5) return "8n";  // Medianamente activado: notas medias
    if (arousal > 0.2) return "4n";  // Poco activado: notas largas
  ...
  private playClusterHarmony(behavior: EmergentBehavior): void {
    ...
    const chord = behavior.entities.slice(0, 4).map((entity, index) =>
  ...
  private playSynchronizationChord(behavior: EmergentBehavior): void {
    ...
    const chord = behavior.entities.slice(0, 5).map((entity, idx) =>
  ...
  private playPropagationArpeggio(behavior: EmergentBehavior): void {
    ...
    const arpeggioNotes = behavior.entities.map((entity, idx) =>
  ...
  private playNarrativeEvent(behavior: EmergentBehavior): void {
    ...
    const chord = behavior.entities.slice(0, 3).map((entity, idx) =>
