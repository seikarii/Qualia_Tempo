...
const BossRenderer: React.FC<BossRendererProps> = ({ boss, gameTime }) => {
  ...
  const attackEffectRef = useRef<THREE.Group>(null);
  ...
  const stressColor = new THREE.Color().setHSL(0, 1, 0.5 + stressIntensity * 0.3);
  ...
  useFrame((state) => {
    ...
    if (bossGroupRef.current) {
      ...
      if (boss.phase === 1) {
        ...
        bossGroupRef.current.position.y = boss.position[1] + Math.sin(time * 0.5) * 0.3;
        bossGroupRef.current.rotation.y += 0.005;
      } else if (boss.phase === 2) {
        // More aggressive movement
        bossGroupRef.current.position.y = boss.position[1] + Math.sin(time * 1.5) * 0.5;
        bossGroupRef.current.position.x = boss.position[0] + Math.cos(time * 0.8) * 0.5;
        bossGroupRef.current.rotation.y += 0.01;
        bossGroupRef.current.rotation.z += Math.sin(time) * 0.01;
      } else {
        // Chaotic final phase movement
        bossGroupRef.current.position.y = boss.position[1] + Math.sin(time * 3) * 0.8;
        bossGroupRef.current.position.x = boss.position[0] + Math.cos(time * 2.1) * 1.0;
    ...
    if (coreMeshRef.current) {
      ...
      if (coreMeshRef.current.material instanceof THREE.MeshBasicMaterial) {
        coreMeshRef.current.material.color = stressColor;
        coreMeshRef.current.material.emissive = stressColor.clone().multiplyScalar(stressIntensity);
      }
    }
    ...
    if (tentaclesRef.current) {
      tentaclesRef.current.children.forEach((tentacle, index) => {
        ...
        tentacle.rotation.z += Math.cos(tentacleTime * 1.2) * 0.015 * stressIntensity;
          ...
          return (
            <group key={i} position={[x, 0, z]} rotation={[0, angle, 0]}>
              ...
              {[...Array(3)].map((_, segmentIndex) => (
                <mesh key={segmentIndex} position={[0, -segmentIndex * 0.5, segmentIndex * 0.3]}>
...
const AttackEffect: React.FC<AttackEffectProps> = ({ phase, color, gameTime }) => {
