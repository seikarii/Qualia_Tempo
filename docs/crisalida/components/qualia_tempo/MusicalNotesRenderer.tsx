    ...
    if (notesGroupRef.current) {
      notesGroupRef.current.children.forEach((child, index) => {
        ...
        if (mesh.material instanceof THREE.MeshBasicMaterial) {
          ...
          mesh.material.color = getNoteColor(noteState.qualia_signature, noteState.timeDiff);
  ...
  return (
    <group ref={notesGroupRef}>
      {noteStates.map((noteState) => {
        ...
        const color = getNoteColor(noteState.qualia_signature, noteState.timeDiff);
        
        return (
          <mesh
            key={noteState.id}
            position={[noteState.position[0], noteState.position[1], noteState.position[2]]}
            onClick={() => {
              if (noteState.isInHitWindow) {
                const accuracy = Math.max(0, 1 - Math.abs(noteState.timeDiff) / 0.5);
          ...
          >
            ...
            <meshBasicMaterial
              ...
              emissive={noteState.isPerfectTiming ? new THREE.Color(0.2, 0.2, 0.2) : new THREE.Color(0, 0, 0)}
            />
...
function getGeometryForType(type: string): THREE.BufferGeometry {
  switch (type) {
    ...
    case 'power':
      return new THREE.CubeGeometry(0.8, 0.8, 0.8);
    case 'grace':
...
const TrailEffect: React.FC<TrailEffectProps> = ({ position, color, signature }) => {
