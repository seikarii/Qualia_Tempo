...
El patrón Stateless View-Logic Pattern establece una separación estricta entre la lógica de cálculo de estado visual y la renderización de componentes. Los componentes React son "tontos" (dumb) y únicamente consumen datos visuales absolutos calculados por servicios especializados. La lógica de animación, transformación y cálculo visual reside exclusivamente en servicios como ViewLogicService, no en componentes. Los componentes llaman a métodos como getBossVisuals() dentro de useFrame y reciben un objeto de datos visuales ya calculados, aplicándolos directamente a objetos Three.js. Este patrón prohíbe realizar cálculos, lógica condicional, o transformaciones de datos de estado del juego dentro del cuerpo de render o useFrame de un componente. Los beneficios incluyen testabilidad unitaria de la lógica visual sin renderizar componentes, reutilización de cálculos visuales en múltiples componentes, optimización y cacheo de cálculos a nivel de servicio, y mantenibilidad donde cambios en lógica visual no afectan la estructura del componente.
...
El decorador `@OnEvent` automatiza la suscripción al EventBus, eliminando la necesidad de llamadas manuales a `eventBus.subscribe()` y `eventBus.unsubscribe()`. Cualquier servicio que utilice este decorador DEBE implementar la interfaz `IBaseService`, y el `ApplicationInitializerService` es responsable de invocar `initialize()` y `cleanup()` para gestionar el ciclo de vida de las suscripciones. El decorador registra automáticamente los métodos decorados como listeners de eventos, con las suscripciones creadas en `initialize()` y limpiadas en `cleanup()`. Los eventos están tipados y validados contra contratos de eventos. El patrón requiere que servicios implementen `IBaseService` para que su ciclo de vida sea gestionado por el `ApplicationInitializerService`, previniendo memory leaks. Los beneficios incluyen eliminación de código boilerplate, gestión automática del ciclo de vida de suscripciones, prevención de memory leaks, y type safety en eventos.
...
Las pruebas de integración de eventos verifican el flujo completo desde la simulación de input del usuario hasta la actualización del estado en el GameStateStore, asegurando que todos los servicios interactúan correctamente a través del EventBus. Esta estrategia permite cobertura de integración completa verificando el flujo completo de datos, detección temprana de regresiones capturando problemas en la comunicación entre servicios, simulación realista usando eventos reales como en producción, y validación de estado confirmando que el store refleja correctamente los cambios.
