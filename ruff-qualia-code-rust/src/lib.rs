                ...
                if module_str.contains("services") {
                    for alias in &import_from.names {
                        ...
                        if name.chars().next().unwrap().is_uppercase()
                            && (name.contains("Service") || name.contains("Engine") || name.contains("Manager")) {
...
impl<'a> Visitor for QLA002Checker<'a> {
    fn visit_stmt(&mut self, stmt: &ast::Stmt) {
        ...
        if let ast::Stmt::ClassDef(ref class_def) = stmt {
            ...
            if self.in_service_class && !func_def.name.starts_with('_') {
                ...
                if !has_decorator {
                    self.diagnostic = Some(Diagnostic::new(
                        QLA002,
                        stmt.range(),
                        format!("Public method '{}' in service class must use at least one architectural decorator (@log_execution, @handle_errors, or @validate_schema).", func_def.name)
                    ));
                }
            }
        }
...
impl<'a> Visitor for QLA003Checker<'a> {
    fn visit_stmt(&mut self, stmt: &ast::Stmt) {
        ...
        // Identify concrete service classes from imports
        if let ast::Stmt::ImportFrom(ref import_from) = stmt {
            if let Some(ref module) = import_from.module {
                if module.as_str().contains("services") {
                    for alias in &import_from.names {
                        let name = alias.name.as_str();
                        if name.chars().next().unwrap().is_uppercase()
                            && (name.contains("Service") || name.contains("Engine") || name.contains("Manager")) {
                            self.concrete_classes.insert(name.to_string());
                        }
                    }
                }
            }
            ...
            if let ast::Expr::Name(ref name) = &call.func {
                if name.id.as_str() == "Depends" {
                    if let Some(first_arg) = call.args.first() {
                        if let ast::Expr::Name(ref dep_name) = first_arg {
                            let dep_class = dep_name.id.as_str();
                            if self.concrete_classes.contains(dep_class) {
                                self.diagnostic = Some(Diagnostic::new(
                                    QLA003,
                                    range,
                                    format!("Route parameter '{}' uses Depends() with concrete class '{}'. Use abstract interface instead.", arg.arg, dep_class)
                                ));
                            }
                        }
                    }
