...
# C    @pytest.mark.asyncio
    async def test_disconnect_client(self, streaming_service):
        ...
        assert mock_websocket not in streaming_service._connectionsive unit tests for WebSocket streaming service
...
class TestStreamingWebService:
    ...
    def mock_rendering_service(self):
        ...
        service.render_frame.return_value = b'\xff\xd8\xff\xe0' + b'\x00' * 1000  # Mock JPEG data
    ...
    @pytest.mark.asyncio
    async def test_start_streaming_initializes_loop(self, streaming_service, mock_rendering_service):
    ...
    @pytest.mark.asyncio
    async def test_streaming_loop_calls_render_and_broadcast(self, streaming_service, mock_rendering_service):
        ...
        with patch.object(streaming_service, '_broadcast_frame') as mock_broadcast, \
