...
(async () => {
  console.log('🚀 Starting Playwright E2E Test for Backend-Frontend Connection');
  ...
  try {
    ...
    if (backendStatus.length === 0) {
      ...
      backendStatus.forEach(status => {
        console.log(`  - ${status.tag}#${status.id}.${status.className}: "${status.text}"`);
      });
    }
    ...
    const connectionKeywords = await page.evaluate(() => {
      ...
      const keywords = ['Backend Disconnected', 'Connected', 'Disconnected', 'Connection', 'backend', 'Backend'];
      const found = [];
    ...
    const appState = await page.evaluate(() => {
      ...
      const initialized = document.querySelector('*[class*="initialized"]') ||
    ...
    console.log(`  Initialized: ${appState.hasInitialized} (${appState.initializedText})`);
    
    // Final assessment
    console.log('\n📊 E2E Test Results:');
    if (connectionKeywords.includes('Backend Disconnected')) {
      console.log('❌ BACKEND CONNECTION FAILED - "Backend Disconnected" found in page');
    } else if (connectionKeywords.includes('Connected') || connectionKeywords.includes('backend')) {
