...
const QualiaTempoHUD: React.FC<QualiaTempoHUDProps> = ({ 
  ...
  const [qualiaOrbs, setQualiaOrbs] = useState<Array<{
    id: string;
    x: number;
    y: number;
    intensity: number;
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        const y2 = Math.cos(Date.now() * 0.001 + i + 1) * 50 + canvas.height / 2;
    ...
    <>
      ...
      >
        ...
        <div className="cyber-gradient border border-cyan-300 px-4 py-2 rounded-lg">
          <NeuralActivityBars activity={qualiaState.intensity} color={intensityColor} />
        </div>
      ...
      >
        ...
        <div className="cyber-gradient border-2 border-cyan-400 px-6 py-4 rounded-xl text-right backdrop-blur-md">
          <div className="text-2xl font-bold font-['Orbitron'] mb-1"
            ...
            {qualiaState.flow > 0.8 ? 'FLOW' : qualiaState.flow > 0.5 ? 'SYNC' : 'LOCK'}
          </div>
          <div className="text-sm text-cyan-300 font-['Orbitron'] uppercase tracking-wider">
            Neural State
          </div>
          ...
          <div className="relative w-16 h-16 mx-auto">
            <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
              ...
              <motion.circle
                ...
                r="40"
                fill="none"
                stroke={playerHealth > 50 ? '#10B981' : playerHealth > 25 ? '#F59E0B' : '#EF4444'}
                strokeWidth="8"
                strokeLinecap="round"
                strokeDasharray="251.2"
                initial={{
