...
const QualiaFieldRenderer: React.FC<QualiaFieldRendererProps> = ({
  ...
  const { positions, colors, sizes } = useMemo(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      sizes[i] =
        (0.1 + musicData.intensity * 0.5) *
        (1 + Math.sin(distance * 0.1) * 0.3);
    }
  ...
  useFrame((state) => {
    ...
    if (fieldMeshRef.current) {
      const positions = fieldMeshRef.current.geometry.attributes.position
        .array as Float32Array;
      const colors = fieldMeshRef.current.geometry.attributes.color
        .array as Float32Array;
      ...
      for (let i = 0; i < positions.length; i += 3) {
        ...
        positions[i + 1] +=
          Math.sin(time * waveFrequency + positions[i] * 0.1) *
          waveAmplitude *
          0.01;
      ...
      fieldMeshRef.current.rotation.y +=
        (musicData.order_influence - 0.5) * 0.005;
    ...
    if (waveRef.current) {
      ...
      for (let i = 0; i < positions.length; i += 3) {
        ...
        positions[i + 1] =
          Math.sin(distance * 0.3 - time * 2) * qualiaField.alpha * 2 +
      ...
      {[...Array(5)].map((_, i) => {
        ...
        return (
          <mesh key={i} position={[x, y, z]}>
            ...
            <meshBasicMaterial
              color={new THREE.Color().setHSL(
                (musicData.emotional_valence + i * 0.2) % 1,
                0.8,
                0.6,
              )}
              transparent={true}
