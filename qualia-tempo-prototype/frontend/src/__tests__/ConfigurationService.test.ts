...
describe('ConfigurationService - QUALIA.CODE v1.1 COMPLIANT', () => {
  ...
  beforeEach(() => {
    ...
    // Configure mocks
    (mocks.mockHttpService.get as Mock).mockResolvedValue('audio:\n  volume: 0.8');
  ...
  describe('loadConfig', () => {
    it('should successfully load and validate configuration', async () => {
      ...
      expect(mocks.mockHttpService.get).toHaveBeenCalledWith('/config/composition-root.yaml');
      expect(mockYamlLoad).toHaveBeenCalledWith(mockYamlText);
      expect(result).toBeDefined();
      expect(typeof result).toBe('object');
      expect(result.compositionRoot).toEqual(mockConfig);
      expect(mocks.mockLogger.info).toHaveBeenCalledWith('Loading configuration from multiple YAML files...');
      expect(mocks.mockLogger.info).toHaveBeenCalledWith('All configurations loaded successfully');
    });
    ...
    it('should handle http service failure', async () => {
      ...
      expect(mocks.mockLogger.error).toHaveBeenCalledWith('Failed to load configuration:', { error: expect.any(Error) });
    });
  ...
  describe('Configuration Access Methods', () => {
    ...
    it('getConfig should throw error when configuration not loaded', () => {
      const newService = new ConfigurationService(mocks.mockLogger, mocks.mockHttpService);
