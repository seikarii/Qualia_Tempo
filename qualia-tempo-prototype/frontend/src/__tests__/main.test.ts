...
describe("Electron Main Process", () => {
  ...
  describe("Window Event Handling", () => {
    ...
    it("should handle window closed event", async () => {
      ...
      const closedCalls = mockBrowserWindow.on.mock.calls.filter(
        (call) => call[0] === "closed",
      );
    ...
    it("should prevent external navigation", async () => {
      ...
      const willNavigateCalls =
        mockBrowserWindow.webContents.on.mock.calls.filter(
  ...
  describe("IPC Handlers", () => {
    ...
    it("should handle get-window-state IPC", async () => {
      ...
      const stateHandler = mockIpcMain.handle.mock.calls.find(
        (call) => call[0] === "get-window-state",
      )[1];
    ...
    it("should handle get-performance-info IPC", async () => {
      ...
      const perfHandler = mockIpcMain.handle.mock.calls.find(
        (call) => call[0] === "get-performance-info",
      )[1];
  ...
  describe("Application Event Handling", () => {
    it("should handle app ready event", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "⚡ Qualia Tempo Engine Initializing...",
      );
      expect(consoleSpy).toHaveBeenCalledWith(
        "🎯 Neural Interface Online - Ready for Synchronization",
      );
    }, 2000);
    ...
    it("should set app user model ID for Windows", async () => {
      ...
      expect(mockApp.setAppUserModelId).toHaveBeenCalledWith(
        "com.qualiatempo.app",
      );
  ...
  describe("Security Features", () => {
    ...
    it("should block external navigation in web contents", async () => {
      ...
      const webContentsHandler = mockApp.on.mock.calls.find(
      ...
      expect(mockContents.setWindowOpenHandler).toHaveBeenCalledWith(
        expect.any(Function),
      );
    });
  });
  ...
  describe("Development Features", () => {
    ...
    it("should handle hot reload in development", async () => {
      ...
      const beforeInputHandler =
        mockBrowserWindow.webContents.on.mock.calls.find(
  ...
  describe("Error Handling", () => {
    ...
    it("should handle graceful shutdown", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "🔄 Initiating Neural Interface Shutdown...",
      );
    });
  });
  ...
  describe("Audio Session Management", () => {
    it("should handle audio session on Windows", async () => {
      ...
      const audioHandler = mockIpcMain.handle.mock.calls.find(
        (call) => call[0] === "set-audio-session",
      )[1];
