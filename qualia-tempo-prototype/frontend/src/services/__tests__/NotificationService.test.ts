...
describe("NotificationService - GOLD.CODE IoC Testing", () => {
  ...
  describe("Priority Queue System", () => {
    ...
    it("should handle high priority notifications first", async () => {
      await notificationService.show("Low priority", "info", 1000, {
        priority: "low",
      });
      await notificationService.show("High priority", "error", 1000, {
        priority: "high",
      });
      await notificationService.show("Normal priority", "warning", 1000, {
        priority: "normal",
      });

      // Check that notifications are processed in priority order
      const logCalls = (mockLogger.info as Mock).mock.calls;
      const notificationCalls = logCalls.filter(
        (call) => call[0] === "Showing notification",
      );
    ...
    it("should limit concurrent notifications", async () => {
      ...
      for (let i = 0; i < 10; i++) {
        promises.push(
          notificationService.show(`Notification ${i}`, "info", 100),
        );
      }
  ...
  describe("Filtering System", () => {
    ...
    it("should allow notifications based on priority override", async () => {
      ...
      await notificationService.updateConfig(config);
      await notificationService.show("High priority warning", "warning", 3000, {
        priority: "high",
      });
