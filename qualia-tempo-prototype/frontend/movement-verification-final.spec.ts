...
test.describe('Movement Controls Verification - Complete Test', () => {
  test('should verify WASD controls work correctly after coordinate fix', async ({ page }) => {
    ...
    page.on('console', (msg) => {
      const text = msg.text();
      if (text.includes('🎵') || text.includes('🎮') || text.includes('PlayerAction') || 
          text.includes('Movement') || text.includes('Dash') || text.includes('Game')) {
